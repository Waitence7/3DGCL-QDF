...
def _static_graph_kwargs(data):
    """Fields unchanged across conformer / geometry views (GIN/GCN need ``x`` + edges)."""
    out = {}
...
class NodeTranslation():
    ...
    def do_trans(self, data):
        if self.method == 'MMFF1':
            ...
            return Data(
                pos=data.max1pos_mmff, smiles=data.smiles, z=data.z,
                energy=data.max1_energy, min_energy=data.min_energy,
                **_static_graph_kwargs(data),
            )
        elif self.method == 'MMFF2':
            return Data(
                pos=data.max2pos_mmff, smiles=data.smiles, z=data.z,
                energy=data.max2_energy, min_energy=data.min_energy,
                **_static_graph_kwargs(data),
            )
        elif self.method == 'MMFF3':
            return Data(
                pos=data.max3pos_mmff, smiles=data.smiles, z=data.z,
                energy=data.max3_energy, min_energy=data.min_energy,
                **_static_graph_kwargs(data),
            )
        elif self.method == 'MMFF4':
            return Data(
                pos=data.max4pos_mmff, smiles=data.smiles, z=data.z,
                energy=data.max4_energy, min_energy=data.min_energy,
                **_static_graph_kwargs(data),
            )
        elif self.method == 'UFF1':
            max1pos_uff = data.max1pos_uff
            return Data(pos=max1pos_uff, smiles=data.smiles, z=data.z, **_static_graph_kwargs(data))
        elif self.method == 'UFF2':
            max2pos_uff = data.max2pos_uff
            return Data(pos=max2pos_uff, smiles=data.smiles, z=data.z, **_static_graph_kwargs(data))
            ...
            return Data(
                pos=rotation.pos, smiles=data.smiles, z=data.z,
                **_static_graph_kwargs(data),
            )
            ...
            return Data(pos=noise, smiles=data.smiles, z=data.z, **_static_graph_kwargs(data))
        #smiles = data.smiles
