...
from dig.sslgraph.method.contrastive.views_fn.translation import _static_graph_kwargs
    ...
    def views_fn(self, batch_data):
        if not isinstance(batch_data, Batch):
            ...
            return Data(
                ...
                **_static_graph_kwargs(batch_data),
            )
        ...
        for k, v in _static_graph_kwargs(batch_data).items():
            setattr(new, k, v)
        new._num_graphs = n_g
        return new
