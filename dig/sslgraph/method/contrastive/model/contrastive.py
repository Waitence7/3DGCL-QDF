...
class Contrastive(nn.Module):
    ...
    def train(self, encoder, data_loader, optimizer, scheduler, epochs, per_epoch_out=False):
        ...
        if self.graph_level and self.proj is not None:
            ...
            lam = float(getattr(self.args, "qdf_aux_lambda", 0.0) or 0.0)
            self.qdf_aux_head = None
            if lam > 0:
                enc_nm = str(getattr(self.args, "encoder", "schnet")).lower()
                if enc_nm in ("gin", "gcn"):
                    in_dim = int(self.args.num_layers) * int(self.args.z_dim)
                else:
                    in_dim = int(self.args.z_dim)
                k_aux = int(getattr(self.args, "qdf_aux_ensemble_k", 1) or 1)
                if k_aux < 1:
                    k_aux = 1
                self.qdf_aux_ensemble_k = k_aux
                self.qdf_aux_head = nn.Linear(in_dim, k_aux).to(self.device)
                _lr = float(getattr(self.args, "p_lr", 1e-3))
                _wd = float(getattr(self.args, "p_weight_decay", 0.0))
                optimizer.add_param_group({
                    "params": self.qdf_aux_head.parameters(),
                    "lr": _lr,
                    "weight_decay": _wd,
                })
        else:
            ...
            for epoch in t:
                ...
                for data in data_loader:
                    ...
                    with _pretrain_autocast(self.device, use_amp):
                        ...
                        if (
                            lam > 0
                            and getattr(self, "qdf_aux_head", None) is not None
                            and hasattr(data, "qdf_aux")
                            and data.qdf_aux is not None
                        ):
                            qdf_t = data.qdf_aux.to(self.device)
                            if qdf_t.dim() == 1:
                                qdf_t = qdf_t.unsqueeze(-1)
                            if torch.isfinite(qdf_t).any():
                                z0 = self._get_embed(encoders[0], views[0].to(self.device))
                                pred = self.qdf_aux_head(z0)
                                if pred.dim() == 1:
                                    pred = pred.unsqueeze(-1)
                                k_t = int(qdf_t.size(-1))
                                k_p = int(pred.size(-1))
                                k_use = min(k_t, k_p)
                                ...
                                for kk in range(k_use):
                                    ...
                                    m = torch.isfinite(tt)
                                    if m.any():
                                        acc = acc + F.mse_loss(pp[m], tt[m])
                                        n_k += 1
                                if n_k > 0:
                                    loss = loss + lam * (acc / float(n_k))
                    loss.backward()
                    optimizer.step()
