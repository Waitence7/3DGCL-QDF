...
class Finetune(object):
    ...
    def __init__(self, args, log_interval=1): #, **kwargs):  #reduction='sum'
        ...
        # ``test_max_heavy_atoms`` kept as legacy alias (same threshold field).
        self.test_max_atoms = getattr(args, "test_max_atoms", None)
        if self.test_max_atoms is None:
            self.test_max_atoms = getattr(args, "test_max_heavy_atoms", None)
        ...
        if self.task_type == 'cls':
            ...
            for i in range(self.n_times):
                ...
                for fold, train_loader, val_loader, test_loader in k_scaffold(self.n_folds, self.dataset,
                                                                              ...
                                                                              getattr(self, "test_max_atoms", None)):
...
class PredictionModel(nn.Module):
    ...
    def __init__(self, encoder, name, num_layers, pred_head, dim, out_dim, dropout):
        ...
        if pred_head == 'schnet':
            # Match ``spherenet`` branch: GIN/GCN graph repr is ``n_layers`` pooled
            # vectors concatenated (see ``encoders.GIN`` / ``GCN``), not ``dim`` alone.
            init_z_dim = num_layers * dim if "gin" in name or "gcn" in name else dim
            self.pred_head = nn.Sequential(
                nn.Linear(init_z_dim, dim // 2),
                ShiftedSoftplus(),
                nn.Dropout(dropout),
...
def _total_atom_count_from_data(data) -> int:
    """Total atom count (H included), aligned with ESOL MoleculeNet ``AddHs`` processing."""
...
def _subset_test_by_max_atoms(test: torch_data.Subset, max_atoms: int) -> torch_data.Subset:
    """Keep only test samples with total atom count (incl. H) <= ``max_atoms``."""
    ma = int(max_atoms)
    kept = [
        int(test.indices[i])
        for i in range(len(test))
        if _total_atom_count_from_data(test[i]) <= ma
    ]
    return torch_data.Subset(test.dataset, kept)


def k_scaffold(n_folds, dataset, batch_size, task_type, seed, test_max_atoms=None):
    ...
    # When ``test_max_atoms`` filters the scaffold test set, many folds end up
    # with a legitimately small test split (e.g. ESOL ≤14 total atoms) while
    # val/train remain large. Applying the same ``min_split_size`` floor to
    # ``test`` then makes splits almost impossible (100× seed retries → error).
    if test_max_atoms is not None:
        min_test_size = 0  # require only non-empty test after atom filtering
    else:
        min_test_size = min_split_size
    while i < n_folds:
        ...
        if test_max_atoms is not None:
            test = _subset_test_by_max_atoms(test, test_max_atoms)
            print(
                f"[k_scaffold] test subset: total_atoms≤{test_max_atoms} "
                f"(RDKit AddHs+GetNumAtoms) → {len(test)}/{n_test_before} molecules",
                flush=True,
            )
        if len(val) <= min_split_size or len(test) <= min_test_size:
            rejections_for_fold += 1
            print(f"[k_scaffold] fold {i + 1} split rejected "
                  f"(train={len(train)}, val={len(val)}, test={len(test)}; "
                  f"need val>{min_split_size} and test>{min_test_size}); "
