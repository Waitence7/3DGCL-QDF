...
Rust backend (requires ``qdf_io`` built via maturin):
    ...
    # rust-legacy: prefer OLD native file; else qdf_io.preprocess_batch_rust_legacy
    QDF_IO_LEGACY_NATIVE_PATH=/path/to/old/libqdf_io.so \\
...
def _load_preprocess_batch_rust_from_native_path(path: Path):
    """Load a **pre-change** ``qdf_io`` extension from disk; return ``preprocess_batch_rust``.

    Old and new extensions both export the same symbol name; the on-disk file decides
    which implementation runs.
    """
    fqname = "qdf_io._native_legacy_external_bench"
    sys.modules.pop(fqname, None)
    spec = importlib.util.spec_from_file_location(fqname, path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load legacy native from {path}")
    ...
    try:
        ...
        sys.modules.pop(fqname, None)
        raise RuntimeError(f"failed to exec_module legacy native {path}: {e}") from e
    if not hasattr(mod, "preprocess_batch_rust"):
        sys.modules.pop(fqname, None)
        raise RuntimeError(
            f"{path} has no preprocess_batch_rust (expected an OLD qdf_io cdylib/pyd)"
        )
    return mod.preprocess_batch_rust
...
def _print_step_table(
    ...
    *,
    ...
    if backend in ("rust", "rust-legacy"):
...
def _qdf_io_package_path() -> str:
    try:
        import qdf_io  # noqa: WPS433

        p = getattr(qdf_io, "__file__", "") or ""
...
def time_preprocess_pipeline(
    ...
    *,
    ...
    has_property: bool,
    no_save: bool,
    rust_legacy_native_path: Path | str | None = None,
) -> tuple[float, dict[str, float], int, Path | None, dict[str, str]]:
    """Run the preprocess loop once.

    Returns ``(wall_s, timings, n_mol, save_dir_or_None, bench_meta)``.
    ``bench_meta`` is always present so callers can record **which** native entrypoint ran
    (external old ``.so``'s ``preprocess_batch_rust`` vs in-tree ``preprocess_batch_rust_legacy`` —
    never silently substituting one for the other).

    For ``backend == "rust-legacy"``: if ``rust_legacy_native_path`` / ``QDF_IO_LEGACY_NATIVE_PATH``
    points to a valid **pre-change** ``.so``/``.pyd``, load ``preprocess_batch_rust`` from it;
    otherwise use **only** ``qdf_io.preprocess_batch_rust_legacy`` (in-tree legacy pipeline;
    never ``preprocess_batch_rust``). Rebuild the extension if the symbol is missing.
    """
    ...
    rust_batch_fn = None
    legacy_native_so_resolved = ""
    uses_external_legacy_native_so = "n/a"
    if backend == "rust":
        try:
            import qdf_io  # noqa: WPS433
        ...
        rust_batch_fn = qdf_io.preprocess_batch_rust
    elif backend == "rust-legacy":
        raw = rust_legacy_native_path or os.environ.get("QDF_IO_LEGACY_NATIVE_PATH")
        legacy_path: Path | None = None
        if raw:
            legacy_path = Path(raw).expanduser().resolve()
            if not legacy_path.is_file():
                legacy_path = None
        if legacy_path is not None:
            try:
                rust_batch_fn = _load_preprocess_batch_rust_from_native_path(legacy_path)
            except RuntimeError:
                save_dir_ctx.cleanup()
                raise
            legacy_native_so_resolved = str(legacy_path)
            uses_external_legacy_native_so = "yes"
        else:
            try:
                import qdf_io  # noqa: WPS433
            except Exception as e:  # pragma: no cover
                save_dir_ctx.cleanup()
                raise RuntimeError(
                    "backend=rust-legacy: set QDF_IO_LEGACY_NATIVE_PATH / rust_legacy_native_path "
                    "to an OLD native .so/.pyd, or install qdf_io with preprocess_batch_rust_legacy "
                    "(maturin develop --release).\n"
                    f"Import error: {e}"
                ) from e
            leg = getattr(qdf_io, "preprocess_batch_rust_legacy", None)
            if leg is None:
                save_dir_ctx.cleanup()
                raise RuntimeError(
                    "backend=rust-legacy: no OLD native path given and qdf_io has no "
                    "preprocess_batch_rust_legacy (in-tree legacy uses process_one_legacy). "
                    "Rebuild from QuantumDeepField_molecule/qdf_io:\n"
                    "  maturin develop --release"
                )
            rust_batch_fn = leg
            uses_external_legacy_native_so = "no"
    ...
    def run_loop() -> None:
        ...
        def flush_rust_chunk(buf: list[dict]) -> None:
            ...
            for mol, (dm_orb, pot, n_field) in zip(buf, outs):
                idx = mol["idx"]
        ...
        if backend in ("rust", "rust-legacy") and rust_buf:
            flush_rust_chunk(rust_buf)
    ...
    if backend == "numpy":
        bench_meta = {
            "uses_external_legacy_native_so": "n/a",
            "legacy_native_so_path": "",
            "rust_batch_entrypoint": "pure_python_numpy",
            "rust_batch_fn_repr": "",
            "qdf_io_package_path": "",
            "frozen_pre_agent_note": "",
        }
    elif backend == "rust":
        bench_meta = {
            "uses_external_legacy_native_so": "n/a",
            "legacy_native_so_path": "",
            "rust_batch_entrypoint": "qdf_io.preprocess_batch_rust",
            "rust_batch_fn_repr": repr(rust_batch_fn),
            "qdf_io_package_path": _qdf_io_package_path(),
            "frozen_pre_agent_note": "",
        }
    else:
        # rust-legacy
        if uses_external_legacy_native_so == "yes":
            entry = "external_old_native.preprocess_batch_rust"
        else:
            entry = "qdf_io.preprocess_batch_rust_legacy"
        bench_meta = {
            "uses_external_legacy_native_so": uses_external_legacy_native_so,
            "legacy_native_so_path": legacy_native_so_resolved,
            "rust_batch_entrypoint": entry,
            "rust_batch_fn_repr": repr(rust_batch_fn),
            "qdf_io_package_path": (
                _qdf_io_package_path() if uses_external_legacy_native_so == "no" else ""
            ),
            "frozen_pre_agent_note": "",
        }
    print(
        "bench_meta:",
        " | ".join(f"{k}={bench_meta[k]!r}" for k in sorted(bench_meta)),
    )

    return total_wall, dict(timings), n_molecules, save_dir_out, bench_meta
...
def main():
    ...
    parser.add_argument(
        ...
        help="Which geometry backend to benchmark. 'numpy' mirrors train/preprocess.py; "
             "'rust' uses the current qdf_io.preprocess_batch_rust; 'rust-legacy' loads an OLD "
             "native's preprocess_batch_rust if --rust-legacy-native-path / QDF_IO_LEGACY_NATIVE_PATH "
             "points to a file, else **only** qdf_io.preprocess_batch_rust_legacy (never the optimized "
             "preprocess_batch_rust as a stand-in).",
    )
    parser.add_argument(
        "--rust-legacy-native-path",
        type=Path,
        default=None,
        help="For --backend rust-legacy: path to the pre-change qdf_io native (.so / .pyd). "
             "Overrides QDF_IO_LEGACY_NATIVE_PATH when set.",
    )
    parser.add_argument(
        "--rust-batch-size",
        type=int,
        default=64,
        help="Molecule batch size for --backend rust or rust-legacy.",
    )
    parser.add_argument(
        "--compare",
        action="store_true",
    ...
    legacy_native: Path | None = args.rust_legacy_native_path
    if legacy_native is None and os.environ.get("QDF_IO_LEGACY_NATIVE_PATH"):
        legacy_native = Path(os.environ["QDF_IO_LEGACY_NATIVE_PATH"]).expanduser()
    ...
    if args.compare:
        ...
        print("--- numpy ---")
        try:
            wall_np, timings_np, n_molecules, save_dir_np, _meta_np = time_preprocess_pipeline(
                blocks,
                backend="numpy",
                ...
                rust_legacy_native_path=legacy_native,
            )
        ...
        print("\n--- rust ---")
        try:
            wall_rs, timings_rs, _, save_dir_rs, _meta_rs = time_preprocess_pipeline(
                blocks,
                backend="rust",
                rust_batch_size=args.rust_batch_size,
                basis_set=args.basis_set,
    ...
    if args.backend in ("rust", "rust-legacy") and args.rust_batch_size < 1:
    ...
    try:
        wall, timings, n_molecules, save_dir_out, bench_meta = time_preprocess_pipeline(
            blocks,
    ...
    print("\n--- bench binding (which code ran) ---")
    for k in sorted(bench_meta):
        print(f"  {k}: {bench_meta[k]}")
