#!/usr/bin/env python3
"""Frozen ``profile_preprocess`` from before external OLD-native loading was added.

``rust-legacy`` uses **only** ``qdf_io.preprocess_batch_rust_legacy`` (never
``preprocess_batch_rust``). ``rust_legacy_native_path`` / ``QDF_IO_LEGACY_NATIVE_PATH``
are accepted for API compatibility with the current script but are ignored.

Shipped beside ``profile_preprocess.py`` for notebook A/B.
"""
...
def time_preprocess_pipeline(
    ...
    *,
    ...
    ``rust-legacy`` uses **only** ``qdf_io.preprocess_batch_rust_legacy`` (never an external
    ``.so``). ``rust_legacy_native_path`` / env are accepted for API compatibility but **ignored**;
    ``bench_meta`` records that so tables never imply an external old binary was used.
    """
    ...
    if backend == "rust":
        ...
        if rust_legacy_native_path:
            p = Path(rust_legacy_native_path).expanduser()
            ignored_legacy_kw = str(p.resolve()) if p.exists() else f"(not a file) {p}"
        elif os.environ.get("QDF_IO_LEGACY_NATIVE_PATH") or os.environ.get("LEGACY_NATIVE_PATH"):
            ignored_legacy_kw = "(env QDF_IO_LEGACY_NATIVE_PATH / LEGACY_NATIVE_PATH set — ignored)"
        try:
            import qdf_io  # noqa: WPS433
        except Exception as e:  # pragma: no cover
            save_dir_ctx.cleanup()
            raise RuntimeError(
                "backend=rust-legacy requires qdf_io. Build from QuantumDeepField_molecule/qdf_io:\n"
                "  maturin develop --release\n"
                f"Original import error: {e}"
            ) from e
        ...
        if leg is None:
            save_dir_ctx.cleanup()
            raise RuntimeError(
                "backend=rust-legacy: qdf_io has no preprocess_batch_rust_legacy "
                "(in-tree legacy uses process_one_legacy). Rebuild from "
    ...
    if backend == "numpy":
        ...
        bench_meta = {
            ...
            "legacy_native_so_path": "",
            "rust_batch_entrypoint": "qdf_io.preprocess_batch_rust_legacy",
            "rust_batch_fn_repr": repr(rust_batch_fn),
            "qdf_io_package_path": _qdf_io_package_path(),
            "frozen_pre_agent_note": (
                "profile_preprocess_pre_agent.py: never loads external OLD .so; "
                "rust_legacy_native_path_kw_or_env_ignored="
                + (ignored_legacy_kw or "(none)")
            ),
        }
    print(
        "bench_meta:",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Which geometry backend to benchmark. 'numpy' mirrors train/preprocess.py; "
             "'rust' uses the current qdf_io.preprocess_batch_rust; 'rust-legacy' uses an OLD "
             "native preprocess_batch_rust if --rust-legacy-native-path / QDF_IO_LEGACY_NATIVE_PATH "
             "is set, else qdf_io.preprocess_batch_rust_legacy.",
    )
    parser.add_argument(
