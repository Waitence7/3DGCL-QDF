...
``--source qdf --pred-csv <path>``
    ...
    * ``smiles,slot,homo,lumo``   (legacy HOMO/LUMO checkpoint)
        score uses any python expression over ``(homo, lumo, gap)`` where
        ``gap = lumo - homo`` (HOMO–LUMO gap in the CSV's energy units).
        Default ``--score-expr "gap"`` so a **larger** gap ⇒ **higher** raw
        score (common optical-gap proxy). On RDKit MMFF rotamers QDF
        predictions only differ by ~1 meV, so always pair with
        ``--normalize zscore``.
    ...
    * ``smiles,slot,energy``      (atomization-energy checkpoint, **recommended**)
    ...
    Example score expressions::
        ...
        # homolumo CSV (``gap`` is precomputed as lumo - homo)
        --score-expr "gap"                     # larger gap ⇒ higher score (default)
        --score-expr "-(lumo - homo)"          # legacy (= -gap; reverses ordering)
...
def read_qdf_slot_rows(csv_path: Path) -> tuple[str, dict, dict[str, dict[int, tuple[float, ...]]]]:
    """Parse one ``qdf_mmff_predict.py`` CSV into ``(prop, spec, rows)``."""
    with csv_path.open("r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if slot not in (1, 2, 3, 4):
                continue
            rows.setdefault(smi, {})[slot] = tuple(float(row[c]) for c in value_cols)
    return prop, spec, rows


def merge_qdf_slot_rows(
    tables: list[tuple[str, dict, dict[str, dict[int, tuple[float, ...]]]]],
    *,
    how: str = "mean",
) -> tuple[str, dict, dict[str, dict[int, tuple[float, ...]]]]:
    """Average / median per (smiles, slot) across QDF CSVs (same schema).

    Only SMILES that have all four MMFF slots in **every** CSV are kept.
    """
    if not tables:
        raise ValueError("merge_qdf_slot_rows: empty tables")
    prop0, spec0, _ = tables[0]
    for prop, spec, _rows in tables[1:]:
        if prop != prop0 or spec["value_cols"] != spec0["value_cols"]:
            raise SystemExit(
                "ensemble QDF CSVs must share the same schema "
                f"(got {prop0!r} vs {prop!r} or different value columns)"
            )
    how = str(how).strip().lower()
    if how not in ("mean", "median"):
        raise SystemExit("--qdf-ensemble-reduce must be mean or median")

    common: set[str] | None = None
    for _p, _s, rows in tables:
        ok = {
            smi for smi, sl in rows.items()
            if set(sl.keys()) == {1, 2, 3, 4}
        }
    ...
    for smi in sorted(common):
        slot_map: dict[int, tuple[float, ...]] = {}
        for slot in (1, 2, 3, 4):
            stacks: list[list[float]] = []
            for _p, _s, rows in tables:
                stacks.append(list(rows[smi][slot]))
            arr = torch.tensor(stacks, dtype=torch.float32)
            if how == "mean":
                fused = arr.mean(dim=0).tolist()
            else:
                fused = arr.median(dim=0).values.tolist()
            slot_map[slot] = tuple(float(x) for x in fused[:dim])
        merged[smi] = slot_map

    print(
        f"[qdf:ensemble:{how}] merged {len(tables)} CSVs -> {len(merged)} molecules "
        "(SMILES with 4 slots in every file)"
    )
    return prop0, spec0, merged


def _slot_rows_to_weights(
    rows: dict[str, dict[int, tuple[float, ...]]],
    *,
    prop: str,
    spec: dict,
...
def qdf_weights(
    ...
    back to the schema's default (``gap`` for homolumo,
    ...
    * ``"rank"``   : replace scores by their per-molecule rank ``0..K-1``,
                     ...
                     magnitude).
    """
    prop, spec, rows = read_qdf_slot_rows(csv_path)
    if score_expr is None:
        score_expr = spec["default_score_expr"]
    out, _skipped, _nc = _slot_rows_to_weights(
        rows, prop=prop, spec=spec, score_expr=score_expr,
        kT=kT, normalize=normalize,
    )
    return out, prop, score_expr


def qdf_weights_ensemble(
    csv_paths: list[Path],
    kT: float,
    score_expr: str | None,
    normalize: str = "zscore",
    *,
    ...
    """Merge several QDF prediction CSVs (mean/median per slot), then softmax."""
    tables = [read_qdf_slot_rows(Path(p)) for p in csv_paths]
    prop, spec, merged = merge_qdf_slot_rows(tables, how=reduce)
    if score_expr is None:
        score_expr = spec["default_score_expr"]
    out, _skipped, _nc = _slot_rows_to_weights(
        merged, prop=prop, spec=spec, score_expr=score_expr,
        kT=kT, normalize=normalize,
    )
    return out, prop, score_expr
...
def main() -> int:
    ...
    ap.add_argument("--pred-csv", type=Path, action="append", default=None,
                    help="QDF prediction CSV from qdf_mmff_predict.py. "
                         "Pass multiple times to ensemble (mean/median per slot) "
                         "before softmax; all files must share the same schema.")
    ap.add_argument(
        "--qdf-ensemble-reduce", choices=("mean", "median"), default="mean",
        help="When multiple --pred-csv are given: per-(smiles,slot) reduction.",
    )
    ap.add_argument(
        ...
        help="Python expression producing the per-slot score; passed to "
             ...
             "Default per schema: 'gap' (= lumo - homo, larger gap = higher) "
    ...
    if args.source == "boltzmann":
        ...
        if not args.pred_csv:
            raise SystemExit("--pred-csv is required for --source qdf (repeat for ensemble)")
        if len(args.pred_csv) == 1:
            weights, qdf_property, score_expr_used = qdf_weights(
                args.pred_csv[0], kT=args.kT, score_expr=args.score_expr,
                normalize=args.normalize,
            )
        else:
            weights, qdf_property, score_expr_used = qdf_weights_ensemble(
                args.pred_csv, kT=args.kT, score_expr=args.score_expr,
                normalize=args.normalize, reduce=args.qdf_ensemble_reduce,
            )
    ...
    payload = {
        ...
        "pred_csv": [str(p) for p in args.pred_csv] if args.pred_csv else None,
        "qdf_ensemble": (
            {"n_csv": len(args.pred_csv), "reduce": args.qdf_ensemble_reduce}
            if args.pred_csv and len(args.pred_csv) > 1 else None
        ),
